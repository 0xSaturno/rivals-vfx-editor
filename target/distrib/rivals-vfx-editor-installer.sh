...
APP_NAME="rivals-vfx-editor"
APP_VERSION="1.1.0"
# Look for GitHub Enterprise-style base URL first
if [ -n "${RIVALS_VFX_EDITOR_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$RIVALS_VFX_EDITOR_INSTALLER_GHE_BASE_URL"
else
    INSTALLER_BASE_URL="${RIVALS_VFX_EDITOR_INSTALLER_GITHUB_BASE_URL:-https://github.com}"
fi
if [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URL="$INSTALLER_DOWNLOAD_URL"
else
    ARTIFACT_DOWNLOAD_URL="${INSTALLER_BASE_URL}/0xSaturno/rivals-vfx-editor/releases/download/v1.1.0"
fi
PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
if [ -n "${RIVALS_VFX_EDITOR_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$RIVALS_VFX_EDITOR_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${RIVALS_VFX_EDITOR_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${RIVALS_VFX_EDITOR_UNMANAGED_INSTALL:-}"
...
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.28.2"},"source":{"app_name":"rivals-vfx-editor","name":"rivals-vfx-editor","owner":"0xSaturno","release_type":"github"},"version":"1.1.0"}
EORECEIPT
RECEIPT_HOME="${XDG_CONFIG_HOME:-$HOME/.config}/rivals-vfx-editor"

usage() {
    # print help (this cat/EOF stuff is a "heredoc" string)
    cat <<EOF
rivals-vfx-editor-installer.sh

The installer for rivals-vfx-editor 1.1.0

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/0xSaturno/rivals-vfx-editor/releases/download/v1.1.0
...
USAGE:
    rivals-vfx-editor-installer.sh [OPTIONS]
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set RIVALS_VFX_EDITOR_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in 
        "rivals-vfx-editor-x86_64-pc-windows-msvc.zip")
            _arch="x86_64-pc-windows-msvc"
            _zip_ext=".zip"
            _checksum_style="sha256"
            _checksum_value="c30b5ab9698c2ce31021c9054cc5d497762c71d3be577090e05b14b1e354e225"
            _bins="rivals-vfx-editor.exe"
            _bins_js_array='"rivals-vfx-editor.exe"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-pc-windows-msvc")
            _archive="rivals-vfx-editor-x86_64-pc-windows-msvc.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
...
install() {
    ...
    if [ -n "${RIVALS_VFX_EDITOR_INSTALL_DIR:-}" ]; then
        _force_install_dir="$RIVALS_VFX_EDITOR_INSTALL_DIR"
